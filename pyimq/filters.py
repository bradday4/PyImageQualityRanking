...
def get_common_options(parser):
    ...
    group.add_argument("--show-plots", dest="show_plots", action="store_true")
    return parser
...
class LocalImageQuality(Filter):
    ...
    def set_smoothing_kernel_size(self, size):
        ...
        if self.physical is True:
            ...
            assert (
                self.kernel_size[0] < self.dimensions[0]
                and self.kernel_size[1] < self.dimensions[1]
